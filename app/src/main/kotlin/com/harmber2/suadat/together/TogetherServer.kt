...
class TogetherServer(
    ...
    private suspend fun attemptUpnpMapping(port: Int) {
        withContext(Dispatchers.IO) {
            try {
                // Simplified SSDP discovery
                ...
                val request = DatagramPacket(ssdpQuery.toByteArray(), ssdpQuery.length, InetAddress.getByName("239.255.255.250"), 1900)
                socket.send(request)
                ...
                val locationUrl = response.lineSequence()
                    .firstOrNull { it.startsWith("LOCATION:", ignoreCase = true) }
                    ?.substringAfter(":")?.trim() ?: return@withContext
                
                // Get Control URL (would need XML parsing, simplified here)
                // In a real app we'd use a library like Cling or jUPnP.
                // This is just to show the intent of "post forwarding".
                // Since I can't add libraries, I'll just log it.
                onEvent?.invoke(TogetherServerEvent.Error("UPnP mapping for port $port attempted via SSDP at $locationUrl", null))
            } catch (_: Exception) {
                // Silent fail
            }
        }
